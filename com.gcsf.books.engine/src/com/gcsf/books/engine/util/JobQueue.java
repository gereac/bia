...
public class JobQueue {
  ...
  public static final QualifiedName NO_IMMEDIATE_ERROR_PROMPT_PROPERTY = new QualifiedName(
      "org.eclipse.ui.workbench.progress", "delayErrorPrompt"); //$NON-NLS-1$ //$NON-NLS-2$
  ...
  private final AtomicInteger fTotalWork = new AtomicInteger(0); // Number of

  // Tasks in
  // Total

  private final AtomicInteger fWorkDone = new AtomicInteger(0); // Number of

  // finished
  // Tasks

  private final AtomicInteger fProgressShown = new AtomicInteger(0); // Number

  // of
  // Progress
  // Shown

  private final AtomicInteger fProgressBuf = new AtomicInteger(0); // Buffer for

  // the
  // Progress
  // Monitor

  private final AtomicInteger fScheduledJobs = new AtomicInteger(0); // Count
   ...
   * 
   * @param name
   *          A human-readable name that is displayed in the Progress-View while
   *          the Queue is processed.
   * @param maxConcurrentJobs
   *          The maximum number of concurrent running Tasks.
   * @param maxQueueSize
   *          The maximum number of tasks that this queue will accept before
   *          blocking.
   * @param showProgress
   *          If TRUE, show Progress of Jobs running from Queue.
   * @param progressDelay
   *          The time in milliseconds to wait before showing any progress. This
   *          is useful in case the Tasks finish very quickly. Setting it to 0
   *          will show Progress instantly with no delay.
   */
  public JobQueue(String name, int maxConcurrentJobs, int maxQueueSize,
      boolean showProgress, int progressDelay) {
    this(name, name, maxConcurrentJobs, maxQueueSize, showProgress,
        progressDelay);
  }
   ...
   *          the Queue is processed.
   * @param taskPrefix
   *          A human-readable prefix that is shown before the name of a task
   ...
   *          will show Progress instantly with no delay.
   */
  public JobQueue(String globalName, String taskPrefix, int maxConcurrentJobs,
      int maxQueueSize, boolean showProgress, int progressDelay) {
  ...
  /**
   * @param isUnknownProgress
   *          Sets the progress reporting of the progress Job used for this
   *          {@link JobQueue} to {@link IProgressMonitor#UNKNOWN} if
   *          <code>true</code>.
   */
   ...
   * 
   * @param joinRunning
   *          If <code>TRUE</code>, join the running Jobs that are not yet done.
   * @param seal
   *          if <code>true</code> this queue is sealed and no tasks will ever
   ...
   * 
   * @param task
   *          The Task to check for being queued in this Queue.
   ...
   * 
   * @param task
   *          The Task to add into this Queue.
   * @return {@code true} if all the tasks were scheduled or {@code false} if
   *         some tasks were not scheduled because the current thread was
   *         interrupted.
   */
   ...
   * @return {@code true} if all the tasks were scheduled or {@code false} if
   *         some tasks were not scheduled because the current thread was
   *         interrupted or this queue is sealed.
   */
        ...
        if (!monitor.isCanceled()) {
          SafeRunner.run(new LoggingSafeRunnable() {
            public void run() throws Exception {
              ...
              if (status.getSeverity() == IStatus.ERROR
                ...
                if (EngineActivator.getDefault() != null)
                  EngineActivator.getDefault().getLog().log(status);
              }
            }
          });
          ...
          if (!monitor.isCanceled()) {
            ...
            if (workDifference > 0 && fProgressJobScheduled) {
              int progress = (int) ((TOTAL_TASK_WORK_LOAD - fProgressShown
                  .get()) / workDifference);
  ...
  private Job createProgressJob() {
    return new Job(fName) {
      ...
      protected IStatus run(IProgressMonitor monitor) {
        ...
        if (!internalIsEmpty())
          monitor.beginTask(fName,
              fIsUnknownProgress ? IProgressMonitor.UNKNOWN
                  : (int) TOTAL_PROGRESS_WORK_LOAD);
        ...
        while (!monitor.isCanceled() && !internalIsEmpty()) {
          ...
          if (fCurrentTask != null
              && ((fLastWorkDone != fWorkDone.get()) || !StringUtils
                  .isSet(fLastTask))) {
  ...
  private String formatTask() {
    ...
    Object[] bindings = Arrays
        .asList(fTaskPrefix, String.valueOf((workDone != 0 ? workDone : 1)),
            String.valueOf(fTotalWork.get()),
            fCurrentTask.replaceAll("&", "&&")).toArray(); //$NON-NLS-1$//$NON-NLS-2$
