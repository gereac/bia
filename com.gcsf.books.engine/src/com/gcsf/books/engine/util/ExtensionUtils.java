...
public class ExtensionUtils {
   ...
   *          The fully qualified identifier of the extension point to use (e.g.
   *          "org.rssowl.core.ApplicationLayer")
   ...
   *         extension point. Third-Party contributions will be chosen over the
   *         default contribution.
   * @throws IllegalStateException
   *           if no contribution was found.
   */
   ...
   *          The default executable that should be returned if no contribution
   *          was found.
   * @return Returns the result of creating an executable extension from the
   *         "class" attribute of the first contribution that matches the given
   *         extension point. Third-Party contributions will be chosen over the
   *         default contribution. If no contribution is found, the default is
   *         returned if provided.
   * @throws IllegalStateException
   *           if no contribution was found and the default executable is
   *           <code>NULL</code>.
   */
  public static Object loadSingletonExecutableExtension(String extensionPoint,
      Object defaultExecutable) {
      ...
      for (IConfigurationElement element : elements) {
        ...
        if (!element.getNamespaceIdentifier().contains(RSSOWL_NAMESPACE)) {
          try {
            ...
            EngineActivator.getDefault().getLog().log(e.getStatus());
          }
        }
      }
    }
    ...
    throw new IllegalStateException(
        "Unable to load contributions for " + extensionPoint); //$NON-NLS-1$
  }
}