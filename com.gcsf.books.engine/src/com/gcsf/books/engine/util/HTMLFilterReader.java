...
public class HTMLFilterReader extends Reader {
  ...
  static {
    ...
    final String[] entityName = {
        "zwnj", "aring", "gt", "yen", "ograve", "Chi", "delta", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
  ...
  private boolean isIdChar(int ch) {
    ...
    return isAlpha(ch) || isDigit(ch) || ch == '.' || ch == '-' || ch == '_'
  ...
  private int readNumericEntity() throws IOException {
    ...
    try {
      if (ch == ';' || ch == -1) {
        fNumWhitespace = fSb.length() + 2;// + 2 accounts for &, #, and ;, then,
                                          // take away 1 for the fact that we do
                                          // output a char
  ...
  private int readTag() throws IOException {
    ...
    if (fEscapedTags != null
        && !fEscapedTags.contains(fSb.toString().toLowerCase())) {
  ...
  private int readName(boolean checkEscaped) throws IOException {
    StringBuilder builder = (checkEscaped && fEscapedTags != null) ? new StringBuilder()
        : null;
    int ch = read();
    if (builder != null)
    ...
    if (builder != null
        && !fEscapedTags.contains(builder.substring(0, builder.length() - 1)
            .toLowerCase())) {
      return MISMATCH;
    }
    return MATCH;
      ...
      switch (ch) {
        ...
        case '<':
          ...
          if (ret == MATCH) {
            ...
            fNumWhitespace = (fNumRead - lastNumRead) - 1;// tack on the -1
                                                          // since we are
                                                          // returning a space
                                                          // right now
            return ' ';
